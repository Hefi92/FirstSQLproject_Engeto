/*
 * První WITH generuje èasovì promìnné - první èást CASE WHEN generuje "0" pro pracovní dny a "1" pro víkend.
 * Druhá èást CASE WHEN na základì definice jednotlivých mìsícù generuje roèní období -> 3-5 jaro, 6-8 léto, 9-11 podzim a zbytek zima.
 */
...
/*
 * Druhé WITH generuje podíly jednotlivých náboženství v konkrétních zemích. Každý øádek bere v potaz konkrétní náboženství.
 * Populace pro konkrétní náboženství se dìlí celkovou populací zemì z tabulky economies. Tabulky jsou následnì spojené pomocí "join".
 */
religion AS(
 	SELECT
 	rel.country,
 ...
 /*
  * Další WITH generuje oèekávanou dobu dožití jedntlivých zemí pro rok 2015.
  */
  expectancy_2015 AS (
	SELECT
	country,
...
/*
 * Další WITH generuje oèekávanou dobu dožití jedntlivých zemí pro rok 1965.
 */
	expectancy_1965 AS (
	SELECT
	country,
...
/*
 * Následné WITH generuje prùmìrnou denní teplotu v jednotlivých mìstech. Teplota se generuje od 6:00 do 21?00 hod.
 * Zaokrouhleno na 2 des. místa, teplota pøevedena na datový typ FLOAT. Následnì je ve finální èásti syntaxe napojeno pøes capital_city s tabulkou countries.
 */
...
/*
 * Toto WITH generuje poèet hodin v daném dni, kdyby byly srážky nenulové.
 * Prvnì je spoèítané èas pomocí COUNT a vynásobeno tøemi, kvùli tøíhodinovým intervalùm v tabulce weather.
 * Následnì zaokrouhleno na 2 des. míta, údaj "mm" vymazán a hodnota pøevedena na FLOAT s podmínkou vìtší než nula.
 */
...
/*
 * Poslední napojení pøes WITH generuje maximální sílu vìtru v nárazech bìhem dne.
 * Využito funkce MAX, kdy síla vìtru (gust) pøevedeno na datový typ FLOAT, zaokrouhleno na 2 des. místa.
 * Èasovì omezeno opìt mezi 6:00 a 21:00 hod.
 */
...
/*
 * Následuje vygenerování jednotlivých požadovaných sloupcù, které jsou souèástí jedné tabulky.
 * Panelová data "date" a "country", po kterých následují požadované a seøazené jednotlivé sloupce. 
 * Nìkteré z nich(napø. "gini", "mortality_under5" atd.) bráno již z existujících tabulek a pouze pøipojeno - nebylo tøeba poèítat.
 * Jako základní tabulka brána "countries", na které jsou následnì pomocí "join" napojeny ostatní pomocné tabulky.
 * Omezující podmínky WHERE doplnìny na posledním øádku skriptu.
 */
	SELECT
	poy.date,
	poy.country,
